...
<article id="node-<?php print $node->nid; ?>" class="<?php print implode(' ', $classes); ?> clearfix"<?php print backdrop_attributes($attributes); ?>>
  ...
  <?php if (!empty($node_url)): ?>
    <h4><a href="<?php print $node_url; ?>" rel="bookmark"><?php print $title; ?></a></h4>
  <?php else: ?>
    <h4><?php print $title; ?></h4>
  <?php endif; ?>
  <?php print render($title_suffix); ?>

  <?php print render($content['field_stanford_event_datetime']); ?>
  <?php print render($content['field_stanford_event_location']); ?>
  <div class="content"<?php print backdrop_attributes($content_attributes); ?>>
    <?php
      // We hide the links now so that we can render them later.
      // hide($content['field_stanford_event_datetime']);
      // hide($content['field_stanford_event_location']);
